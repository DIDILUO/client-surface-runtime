...
} from '../models/contentResourceDetail.model'
import { contentResourceBadgeToneMap } from '../models/contentResourceDetail.model'
...
const formatCurrencyValue = (priceInCent: number): string => Math.round(priceInCent / 100).toLocaleString('en-US')
...
const createBannerShell = (resourceId: string): ContentResourceDetailContent => ({
  ...
  relationNote: '当前页面继续按 `resourceType + resourceId` 取数，后续只需要让后台返回同一 banner 资源详情即可。',
})
...
const createActivityShell = (resourceId: string): ContentResourceDetailContent => ({
  ...
  relationNote: '当前页面继续按 `resourceType + resourceId` 取数，后续只需要让后台返回同一活动资源详情即可。',
})
...
const createShellByType = (resourceType: SupportedResourceType, resourceId: string): ContentResourceDetailContent => {
...
const mapBannerResource = (
  resource: Pick<ContentResourceDto, 'resourceId' | 'title' | 'subtitle' | 'status' | 'summary' | 'asset'> & {
    payload: ContentHomeBannerPayloadDto
  },
): ContentResourceDetailContent => {
  const assetSizeLabel = resource.asset ? `${resource.asset.width} × ${resource.asset.height}` : 'ASSET READY'
  ...
  return {
    ...
    summary: resource.summary ?? '当前首页主视觉已经进入正式资源链，后续继续由后台上传成品图并维护同一资源 ID。',
    englishTitle: 'BANNER DETAIL',
    imageUrl: resource.asset?.variants?.detail ?? resource.asset?.variants?.banner ?? resource.asset?.originalUrl ?? '',
    placeholderIconKey: 'box',
    priceLabel: '展示状态',
    priceUnit: '',
    ...
    relationNote: '后续接入真实后台时，这里只需要继续返回 `home_banner` 资源详情，不需要再新增页面。',
  }
}
...
const mapDropResource = (
  ...
  const progressRate =
    resource.payload.supplyCount > 0 ? Math.min(100, Math.max(0, (resource.payload.mintedCount / resource.payload.supplyCount) * 100)) : 0
  ...
  return {
    ...
    englishTitle: 'DROP DETAIL',
    imageUrl: resource.asset?.variants?.detail ?? resource.asset?.variants?.card ?? resource.asset?.originalUrl ?? '',
...
const mapActivityResource = (
  ...
  return {
    ...
    visualTone: resource.payload.tone === 'dark' ? 'slate' : resource.payload.tone === 'light' ? 'sand' : 'aqua',
    badges: [
      { id: 'activity', label: '活动资源', tone: 'cyan' },
      ...(resource.payload.badgeLabel ? [{ id: 'badge', label: resource.payload.badgeLabel, tone: 'amber' as const }] : []),
    ],
    stats: [
      { id: 'status', label: '当前状态', value: resource.status.toUpperCase() },
      { id: 'tone', label: '视觉口径', value: resource.payload.tone.toUpperCase(), caption: 'Tone' },
    ],
...
const mapMarketResource = (
  ...
  const badges: ContentResourceDetailBadge[] = [
    { id: 'market', label: '市场资源', tone: 'slate' },
  ]
  ...
  return {
    ...
    badges,
    stats,
    relationNote: '后续接入真实后台时，这里只需要继续返回 `market_item` 资源详情，不需要再新增页面。',
  }
}

const adaptResource = (resource: ContentResourceDto, fallbackType: SupportedResourceType): ContentResourceDetailContent => {
...
export const createContentResourceDetailShell = (
  ...
  const snapshot = createContentResourceSnapshot(resourceType, resourceId)
  return snapshot ? adaptResource(snapshot, resourceType) : createShellByType(resourceType, resourceId)
}
