/**
 * 文件版本：v0015
 * 更新时间：2026-03-24 17:18:00
 * Encoding: UTF-8
 * 本次更新：触控端默认强制单页链路，多页并行仅在折叠分段能力命中时启用
 */

import {
  HOME_LAYOUT_MIN_DOUBLE_WIDTH,
  HOME_LAYOUT_MIN_NAV_WIDTH,
  HOME_LAYOUT_MIN_TRIPLE_WIDTH,
  HOME_LAYOUT_MODE_DOUBLE_PAGE,
  HOME_LAYOUT_MODE_SINGLE_PAGE,
  HOME_LAYOUT_MODE_SINGLE_PAGE_WITH_NAV,
  HOME_LAYOUT_MODE_TRIPLE_PAGE,
  HOME_LAYOUT_MODE_TRIPLE_PAGE_WITH_STAGE,
  HOME_LAYOUT_STAGE_FILL_THRESHOLD,
  type LayoutMode,
} from '../models/homeShell.model'

export type SegmentPreference = typeof HOME_LAYOUT_MODE_DOUBLE_PAGE | typeof HOME_LAYOUT_MODE_TRIPLE_PAGE | null

export type DevicePointerType = 'touch' | 'mouse' | 'hybrid' | 'unknown'

export interface DeviceCapabilityFlags {
  hasVisualViewport: boolean
  hasWindowSegments: boolean
  hoverCapable: boolean
  pointerType: DevicePointerType
}

export interface ViewportRuntimeContext extends DeviceCapabilityFlags {
  viewportWidth: number
  viewportHeight: number
  safeAreaTop: number
  safeAreaBottom: number
  segmentCount: number
}

export interface HomeShellInsets {
  topInset: number
  bottomInset: number
}

interface HomeShellViewportSnapshot {
  runtimeContext: ViewportRuntimeContext
  segmentPreference: SegmentPreference
}

type ViewportSyncListener = () => void

interface ViewportListenerBindOptions {
  minIntervalMs?: number
}

interface SegmentRect {
  top: number
  left: number
  width: number
  height: number
}

interface UniWindowInfoCandidate {
  safeAreaInsets?: {
    top?: number
    bottom?: number
  }
  statusBarHeight?: number
  safeArea?: {
    top?: number
    bottom?: number
    height?: number
  }
  windowHeight?: number
}

const getNumberOrFallback = (value: number | undefined, fallback: number): number => {
  if (typeof value !== 'number' || !Number.isFinite(value) || value <= 0) {
    return fallback
  }

  return value
}

const HOME_LAYOUT_SWITCH_HYSTERESIS = 12
const HOME_LAYOUT_VIEWPORT_LISTENER_MIN_INTERVAL_MS = 80

const resolveViewportDimensionByPriority = (candidates: number[]): number => {
  for (const candidate of candidates) {
    if (candidate > 0) {
      return Math.round(candidate)
    }
  }
  return 0
}

export const resolveWindowViewportWidth = (currentWindow: Window): number => {
  return resolveViewportDimensionByPriority([
    getNumberOrFallback(currentWindow.innerWidth, 0),
    getNumberOrFallback(currentWindow.visualViewport?.width, 0),
    getNumberOrFallback(currentWindow.document?.documentElement?.clientWidth, 0),
  ])
}

export const resolveWindowViewportHeight = (currentWindow: Window): number => {
  return resolveViewportDimensionByPriority([
    getNumberOrFallback(currentWindow.visualViewport?.height, 0),
    getNumberOrFallback(currentWindow.innerHeight, 0),
    getNumberOrFallback(currentWindow.document?.documentElement?.clientHeight, 0),
  ])
}

export const isSingleLayoutMode = (layoutMode: LayoutMode): boolean => {
  return layoutMode === HOME_LAYOUT_MODE_SINGLE_PAGE || layoutMode === HOME_LAYOUT_MODE_SINGLE_PAGE_WITH_NAV
}

export const isDoubleLayoutMode = (layoutMode: LayoutMode): boolean => {
  return layoutMode === HOME_LAYOUT_MODE_DOUBLE_PAGE
}

export const isTripleLayoutMode = (layoutMode: LayoutMode): boolean => {
  return layoutMode === HOME_LAYOUT_MODE_TRIPLE_PAGE || layoutMode === HOME_LAYOUT_MODE_TRIPLE_PAGE_WITH_STAGE
}

export const canUseDrawerInLayoutMode = (layoutMode: LayoutMode): boolean => {
  return layoutMode === HOME_LAYOUT_MODE_SINGLE_PAGE || layoutMode === HOME_LAYOUT_MODE_SINGLE_PAGE_WITH_NAV
}

export const showNavRailInLayoutMode = (layoutMode: LayoutMode): boolean => {
  return layoutMode === HOME_LAYOUT_MODE_SINGLE_PAGE_WITH_NAV
}

export const showBottomTabbarInLayoutMode = (layoutMode: LayoutMode): boolean => {
  return !isTripleLayoutMode(layoutMode)
}

export interface HomeShellLayoutFlags {
  isSingleMode: boolean
  isDoubleMode: boolean
  isTripleMode: boolean
  canUseDrawer: boolean
  showNavRail: boolean
  showBottomTabbar: boolean
}

export interface HomeShellStageModeClassMap {
  'mode-single': boolean
  'mode-single-nav': boolean
  'mode-double': boolean
  'mode-triple': boolean
}

export interface HomeShellPageGridClassMap {
  'is-single': boolean
  'is-double': boolean
  'is-triple': boolean
}

export interface HomeShellTrackLayoutState extends HomeShellLayoutFlags {
  stageModeClass: HomeShellStageModeClassMap
  pageGridClass: HomeShellPageGridClassMap
}

export const resolveHomeShellLayoutFlags = (layoutMode: LayoutMode): HomeShellLayoutFlags => {
  const isSingleMode = isSingleLayoutMode(layoutMode)
  const isDoubleMode = isDoubleLayoutMode(layoutMode)
  const isTripleMode = isTripleLayoutMode(layoutMode)
  const canUseDrawer = canUseDrawerInLayoutMode(layoutMode)
  const showNavRail = showNavRailInLayoutMode(layoutMode)
  const showBottomTabbar = showBottomTabbarInLayoutMode(layoutMode)

  return {
    isSingleMode,
    isDoubleMode,
    isTripleMode,
    canUseDrawer,
    showNavRail,
    showBottomTabbar,
  }
}

const resolveHomeShellStageModeClassByFlags = (layoutFlags: HomeShellLayoutFlags): HomeShellStageModeClassMap => {
  return {
    'mode-single': layoutFlags.isSingleMode && !layoutFlags.showNavRail,
    'mode-single-nav': layoutFlags.showNavRail,
    'mode-double': layoutFlags.isDoubleMode,
    'mode-triple': layoutFlags.isTripleMode,
  }
}

const resolveHomeShellPageGridClassByFlags = (layoutFlags: HomeShellLayoutFlags): HomeShellPageGridClassMap => {
  return {
    'is-single': layoutFlags.isSingleMode,
    'is-double': layoutFlags.isDoubleMode,
    'is-triple': layoutFlags.isTripleMode,
  }
}

export const resolveHomeShellStageModeClass = (layoutMode: LayoutMode): HomeShellStageModeClassMap => {
  const layoutFlags = resolveHomeShellLayoutFlags(layoutMode)
  return resolveHomeShellStageModeClassByFlags(layoutFlags)
}

export const resolveHomeShellPageGridClass = (layoutMode: LayoutMode): HomeShellPageGridClassMap => {
  const layoutFlags = resolveHomeShellLayoutFlags(layoutMode)
  return resolveHomeShellPageGridClassByFlags(layoutFlags)
}

export const resolveHomeShellTrackLayoutState = (layoutMode: LayoutMode): HomeShellTrackLayoutState => {
  const layoutFlags = resolveHomeShellLayoutFlags(layoutMode)
  const stageModeClass = resolveHomeShellStageModeClassByFlags(layoutFlags)
  const pageGridClass = resolveHomeShellPageGridClassByFlags(layoutFlags)

  return {
    ...layoutFlags,
    stageModeClass,
    pageGridClass,
  }
}

const parseCssLengthToNumber = (rawValue: string | undefined): number => {
  if (typeof rawValue !== 'string') {
    return 0
  }

  const trimmed = rawValue.trim()
  if (!trimmed) {
    return 0
  }

  const parsed = Number.parseFloat(trimmed)
  if (!Number.isFinite(parsed)) {
    return 0
  }

  return Math.max(0, Math.round(parsed))
}

const resolveCssSafeAreaInset = (currentWindow: Window, cssVariableName: string): number => {
  const rootElement = currentWindow.document?.documentElement
  if (!rootElement) {
    return 0
  }

  const computedStyle = currentWindow.getComputedStyle(rootElement)
  return parseCssLengthToNumber(computedStyle.getPropertyValue(cssVariableName))
}

const resolveUniWindowInfo = (): UniWindowInfoCandidate | null => {
  const host = globalThis as typeof globalThis & {
    uni?: {
      getWindowInfo?: () => unknown
    }
  }

  const getWindowInfo = host.uni?.getWindowInfo
  if (typeof getWindowInfo !== 'function') {
    return null
  }

  try {
    const info = getWindowInfo()
    if (!info || typeof info !== 'object') {
      return null
    }

    return info as UniWindowInfoCandidate
  } catch {
    return null
  }
}

const resolveSafeAreaTopFromUni = (windowInfo: UniWindowInfoCandidate | null): number => {
  if (!windowInfo) {
    return 0
  }

  const safeInsetsTop = getNumberOrFallback(windowInfo.safeAreaInsets?.top, 0)
  const statusBarHeight = getNumberOrFallback(windowInfo.statusBarHeight, 0)
  const safeAreaTop = getNumberOrFallback(windowInfo.safeArea?.top, 0)

  return Math.max(safeInsetsTop, statusBarHeight, safeAreaTop)
}

const resolveSafeAreaBottomFromUni = (windowInfo: UniWindowInfoCandidate | null): number => {
  if (!windowInfo) {
    return 0
  }

  const safeInsetsBottom = getNumberOrFallback(windowInfo.safeAreaInsets?.bottom, 0)
  if (safeInsetsBottom > 0) {
    return safeInsetsBottom
  }

  const safeAreaBottom = getNumberOrFallback(windowInfo.safeArea?.bottom, 0)
  const windowHeight = getNumberOrFallback(windowInfo.windowHeight, 0)
  if (safeAreaBottom > 0 && windowHeight > 0) {
    return Math.max(0, Math.round(windowHeight - safeAreaBottom))
  }

  return 0
}

const resolveDevicePointerType = (currentWindow: Window): DevicePointerType => {
  const hasFinePointer =
    currentWindow.matchMedia('(pointer: fine)').matches || currentWindow.matchMedia('(any-pointer: fine)').matches
  const hasCoarsePointer =
    currentWindow.matchMedia('(pointer: coarse)').matches ||
    currentWindow.matchMedia('(any-pointer: coarse)').matches ||
    (typeof currentWindow.navigator?.maxTouchPoints === 'number' && currentWindow.navigator.maxTouchPoints > 0)

  if (hasFinePointer && hasCoarsePointer) {
    return 'hybrid'
  }

  if (hasCoarsePointer) {
    return 'touch'
  }

  if (hasFinePointer) {
    return 'mouse'
  }

  return 'unknown'
}

const detectSegmentPreferenceByRects = (segments: SegmentRect[]): SegmentPreference => {
  if (segments.length < 2) {
    return null
  }

  const sorted = [...segments].sort((left, right) => left.left - right.left)
  const baselineTop = sorted[0]?.top ?? 0
  const baselineHeight = sorted[0]?.height ?? 0
  const isHorizontal = sorted.every((segment) => {
    return Math.abs(segment.top - baselineTop) <= 2 && Math.abs(segment.height - baselineHeight) <= 2
  })

  if (!isHorizontal) {
    return null
  }

  if (sorted.length >= 3) {
    return HOME_LAYOUT_MODE_TRIPLE_PAGE
  }

  if (sorted.length === 2) {
    return HOME_LAYOUT_MODE_DOUBLE_PAGE
  }

  return null
}

const resolveWindowSegments = (targetWindow: Window): SegmentRect[] => {
  const api = (targetWindow as Window & { getWindowSegments?: () => SegmentRect[] }).getWindowSegments
  if (typeof api !== 'function') {
    return []
  }

  try {
    const segments = api()
    if (!Array.isArray(segments)) {
      return []
    }

    return segments
  } catch {
    return []
  }
}

export const detectWindowSegmentPreference = (currentWindow?: Window): SegmentPreference => {
  const targetWindow = currentWindow ?? (typeof window === 'undefined' ? null : window)
  if (!targetWindow) {
    return null
  }

  const segments = resolveWindowSegments(targetWindow)
  if (segments.length === 0) {
    return null
  }

  return detectSegmentPreferenceByRects(segments)
}

export const resolveHomeShellViewportSnapshot = (currentWindow: Window): HomeShellViewportSnapshot => {
  const uniWindowInfo = resolveUniWindowInfo()
  const topInsetFromUni = resolveSafeAreaTopFromUni(uniWindowInfo)
  const bottomInsetFromUni = resolveSafeAreaBottomFromUni(uniWindowInfo)
  const topInsetFromCss = resolveCssSafeAreaInset(currentWindow, '--app-safe-area-top')
  const bottomInsetFromCss = resolveCssSafeAreaInset(currentWindow, '--app-safe-area-bottom')
  const hasWindowSegments =
    typeof (currentWindow as Window & { getWindowSegments?: () => SegmentRect[] }).getWindowSegments === 'function'
  const segmentRects = hasWindowSegments ? resolveWindowSegments(currentWindow) : []
  const segmentCount = segmentRects.length

  return {
    runtimeContext: {
      viewportWidth: resolveWindowViewportWidth(currentWindow),
      viewportHeight: resolveWindowViewportHeight(currentWindow),
      safeAreaTop: Math.max(topInsetFromUni, topInsetFromCss),
      safeAreaBottom: Math.max(bottomInsetFromUni, bottomInsetFromCss),
      hasVisualViewport: Boolean(currentWindow.visualViewport),
      hasWindowSegments,
      segmentCount,
      pointerType: resolveDevicePointerType(currentWindow),
      hoverCapable:
        currentWindow.matchMedia('(hover: hover)').matches || currentWindow.matchMedia('(any-hover: hover)').matches,
    },
    segmentPreference: detectWindowSegmentPreference(currentWindow),
  }
}

export const resolveHomeShellInsets = (runtimeContext: ViewportRuntimeContext): HomeShellInsets => {
  return {
    topInset: Math.max(0, runtimeContext.safeAreaTop),
    bottomInset: Math.max(0, runtimeContext.safeAreaBottom),
  }
}

export const bindHomeShellViewportListeners = (
  currentWindow: Window,
  listener: ViewportSyncListener,
  options?: ViewportListenerBindOptions,
): (() => void) => {
  const minIntervalMs = Math.max(0, Math.round(options?.minIntervalMs ?? HOME_LAYOUT_VIEWPORT_LISTENER_MIN_INTERVAL_MS))
  let animationFrameId: number | null = null
  let delayedTimerId: number | null = null
  let lastEmitTimestamp = 0

  const flushListener = () => {
    const nowTimestamp = Date.now()
    const elapsedMs = nowTimestamp - lastEmitTimestamp

    if (elapsedMs >= minIntervalMs) {
      lastEmitTimestamp = nowTimestamp
      listener()
      return
    }

    if (delayedTimerId !== null) {
      currentWindow.clearTimeout(delayedTimerId)
    }

    delayedTimerId = currentWindow.setTimeout(() => {
      delayedTimerId = null
      lastEmitTimestamp = Date.now()
      listener()
    }, minIntervalMs - elapsedMs)
  }

  const scheduleListener = () => {
    if (animationFrameId !== null) {
      return
    }

    animationFrameId = currentWindow.requestAnimationFrame(() => {
      animationFrameId = null
      flushListener()
    })
  }

  currentWindow.addEventListener('resize', scheduleListener)
  currentWindow.addEventListener('orientationchange', scheduleListener)
  const currentVisualViewport = currentWindow.visualViewport ?? null
  currentVisualViewport?.addEventListener('resize', scheduleListener)

  return () => {
    currentWindow.removeEventListener('resize', scheduleListener)
    currentWindow.removeEventListener('orientationchange', scheduleListener)
    currentVisualViewport?.removeEventListener('resize', scheduleListener)

    if (animationFrameId !== null) {
      currentWindow.cancelAnimationFrame(animationFrameId)
      animationFrameId = null
    }

    if (delayedTimerId !== null) {
      currentWindow.clearTimeout(delayedTimerId)
      delayedTimerId = null
    }
  }
}

const resolveHomeShellLayoutModeByWidthAndSegments = (
  runtimeContext: ViewportRuntimeContext,
  viewportWidth: number,
  segmentPreference: SegmentPreference,
): LayoutMode => {
  const isTouchLikeDevice =
    (runtimeContext.pointerType === 'touch' || runtimeContext.pointerType === 'hybrid') && !runtimeContext.hoverCapable
  const hasFoldableSegmentLayout = runtimeContext.hasWindowSegments && runtimeContext.segmentCount >= 2

  if (isTouchLikeDevice && !hasFoldableSegmentLayout) {
    if (viewportWidth < HOME_LAYOUT_MIN_NAV_WIDTH) {
      return HOME_LAYOUT_MODE_SINGLE_PAGE
    }
    return HOME_LAYOUT_MODE_SINGLE_PAGE_WITH_NAV
  }

  let mode: LayoutMode
  if (viewportWidth < HOME_LAYOUT_MIN_NAV_WIDTH) {
    mode = HOME_LAYOUT_MODE_SINGLE_PAGE
  } else if (viewportWidth < HOME_LAYOUT_MIN_DOUBLE_WIDTH) {
    mode = HOME_LAYOUT_MODE_SINGLE_PAGE_WITH_NAV
  } else if (viewportWidth < HOME_LAYOUT_MIN_TRIPLE_WIDTH) {
    mode = HOME_LAYOUT_MODE_DOUBLE_PAGE
  } else {
    mode = HOME_LAYOUT_MODE_TRIPLE_PAGE
  }

  if (segmentPreference === HOME_LAYOUT_MODE_TRIPLE_PAGE && viewportWidth >= HOME_LAYOUT_MIN_DOUBLE_WIDTH) {
    mode = HOME_LAYOUT_MODE_TRIPLE_PAGE
  } else if (
    segmentPreference === HOME_LAYOUT_MODE_DOUBLE_PAGE &&
    viewportWidth >= HOME_LAYOUT_MIN_NAV_WIDTH &&
    mode === HOME_LAYOUT_MODE_SINGLE_PAGE_WITH_NAV
  ) {
    mode = HOME_LAYOUT_MODE_DOUBLE_PAGE
  }

  if (mode === HOME_LAYOUT_MODE_TRIPLE_PAGE && viewportWidth > HOME_LAYOUT_STAGE_FILL_THRESHOLD) {
    return HOME_LAYOUT_MODE_TRIPLE_PAGE_WITH_STAGE
  }

  return mode
}

const resolveHomeShellLayoutModeWithHysteresis = (
  viewportWidth: number,
  targetMode: LayoutMode,
  previousMode?: LayoutMode,
): LayoutMode => {
  if (!previousMode) {
    return targetMode
  }

  const hysteresis = HOME_LAYOUT_SWITCH_HYSTERESIS

  if (previousMode === HOME_LAYOUT_MODE_SINGLE_PAGE) {
    if (viewportWidth < HOME_LAYOUT_MIN_NAV_WIDTH + hysteresis) {
      return HOME_LAYOUT_MODE_SINGLE_PAGE
    }
    return targetMode
  }

  if (previousMode === HOME_LAYOUT_MODE_SINGLE_PAGE_WITH_NAV) {
    if (
      viewportWidth >= HOME_LAYOUT_MIN_NAV_WIDTH &&
      viewportWidth < HOME_LAYOUT_MIN_DOUBLE_WIDTH + hysteresis
    ) {
      return HOME_LAYOUT_MODE_SINGLE_PAGE_WITH_NAV
    }
    return targetMode
  }

  if (previousMode === HOME_LAYOUT_MODE_DOUBLE_PAGE) {
    if (
      viewportWidth >= HOME_LAYOUT_MIN_DOUBLE_WIDTH &&
      viewportWidth < HOME_LAYOUT_MIN_TRIPLE_WIDTH + hysteresis
    ) {
      return HOME_LAYOUT_MODE_DOUBLE_PAGE
    }
    return targetMode
  }

  if (previousMode === HOME_LAYOUT_MODE_TRIPLE_PAGE) {
    if (
      viewportWidth >= HOME_LAYOUT_MIN_TRIPLE_WIDTH &&
      viewportWidth <= HOME_LAYOUT_STAGE_FILL_THRESHOLD + hysteresis
    ) {
      return HOME_LAYOUT_MODE_TRIPLE_PAGE
    }
    return targetMode
  }

  if (previousMode === HOME_LAYOUT_MODE_TRIPLE_PAGE_WITH_STAGE) {
    if (viewportWidth >= HOME_LAYOUT_STAGE_FILL_THRESHOLD) {
      return HOME_LAYOUT_MODE_TRIPLE_PAGE_WITH_STAGE
    }
    return targetMode
  }

  return targetMode
}

export const resolveHomeShellLayoutMode = (
  runtimeContext: ViewportRuntimeContext,
  viewportWidth: number,
  segmentPreference: SegmentPreference,
  previousMode?: LayoutMode,
): LayoutMode => {
  const targetMode = resolveHomeShellLayoutModeByWidthAndSegments(runtimeContext, viewportWidth, segmentPreference)
  return resolveHomeShellLayoutModeWithHysteresis(viewportWidth, targetMode, previousMode)
}
